...
import static no.nav.syfo.utils.MapUtil.*;
...
public abstract class WS2SykepengesoknadMapper {
    ...
    private static final Function<XMLSykepengesoeknadOppsummering, Oppsummering> xml2Oppsummering = xmlOppsummering ->
            new Oppsummering()
                    .withSporsmalsliste(mapMangetilEn(xmlOppsummering.getGruppe(), g -> g.getId().equals(Seksjon.SOKNAD.name()), xml2Sporsmalsliste))
                    .withAnsvarserklaring(mapMangetilEn(xmlOppsummering.getGruppe(), g -> g.getId().equals(Seksjon.VAER_KLAR_OVER_AT.name()), gruppe2Tilleggstekst))
                    .withBekreftetKorrektInformasjon(mapMangetilEn(xmlOppsummering.getGruppe(), g -> g.getId().equals(Seksjon.BEKREFT_KORREKT_INFORMASJON.name()), gruppe2Sporsmal));
